...
public class GraphicalStrongbox implements CharFigure {
    ...
    public GraphicalStrongbox(CharStream stream, StrongBox strongbox, int width, int height){
        this.stream = stream;
    ...
    public void draw(int relX, int relY) {
        ...
        if (strongbox.getStrongboxResources().contains(new Coin())) {
            ...
            stream.addString(relX + 2 * offsetX,relY + 2 * offsetY, Integer.toString(strongbox.getStrongboxResources().getMaps().stream().filter(map -> map.getResource().equals(new Coin())).mapToInt(MapResources::getCardinality).sum()), ForeColor.ANSI_BRIGHT_WHITE , BackColor.ANSI_BG_YELLOW);
        }

        if (strongbox.getStrongboxResources().contains(new Shield())) {
            ...
            stream.addString(relX + 6 * offsetX,relY + 2 * offsetY, Integer.toString(strongbox.getStrongboxResources().getMaps().stream().filter(map -> map.getResource().equals(new Shield())).mapToInt(MapResources::getCardinality).sum()), ForeColor.ANSI_BRIGHT_WHITE , BackColor.ANSI_BG_BLUE);
        }

        if (strongbox.getStrongboxResources().contains(new Servant())) {
            ...
            stream.addString(relX + 10 * offsetX,relY + 2 * offsetY, Integer.toString(strongbox.getStrongboxResources().getMaps().stream().filter(map -> map.getResource().equals(new Servant())).mapToInt(MapResources::getCardinality).sum()), ForeColor.ANSI_BRIGHT_WHITE , BackColor.ANSI_BG_PURPLE);
        }

        if (strongbox.getStrongboxResources().contains(new Stone())){
            ...
            stream.addString(relX + 14 * offsetX,relY + 2 * offsetY, Integer.toString(strongbox.getStrongboxResources().getMaps().stream().filter(map -> map.getResource().equals(new Stone())).mapToInt(MapResources::getCardinality).sum()), ForeColor.ANSI_BRIGHT_WHITE , BackColor.ANSI_GREY);
        }
